...
from api.schemas.address import AddressSchema, Address, AddressSchemaUpdate
...
async def create_address_for_user(user, address):
    try:
        ...
        if address.inserted_id:
            address = await _get_field_or_404(address.inserted_id, db.address_db, 'address')
            return address
    except Exception:
...
async def append_address_user(user_id, address_data: Address):
    try:
        ...
        if address_op.modified_count:
            return await _get_field_or_404(add_address['_id'], db.address_db, 'address')
...
async def update_address(user_id, address_data: AddressSchemaUpdate, row_id):
    try:
        ...
        if address_op.modified_count:
            return await _get_field_or_404(address_op._id, db.address_db, 'address')
